$(function () {
    $("#jqGrid").jqGrid({
        url: baseURL + 'invoice/fpkjlog/selectWait',
        datatype: "json",
        colModel: [			
			...
			{ label: '录入日期', name: 'lrrq', index: 'lrrq', width: 15 }
        ],
		viewrecords: true,
        height: 385,
        rowNum: 10,
...
var vm = new Vue({
	...
	methods: {
        ...
        fpRetry: function () {
            var fpids = getSelectedRows();
            if(fpids == null){
                return ;
            }
            confirm('确定要反补选中的发票？', function(){
                $.ajax({
                    type: "POST",
                    url: baseURL + "invoice/fpkjlog/fpRetry",
                    contentType: "application/json",
                    data: JSON.stringify(fpids),
                    success: function(r){
                        if(r.code == 0){
                            alert('操作成功', function(){
                                vm.reload();
